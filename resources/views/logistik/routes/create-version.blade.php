...
@section('title', 'Create Route Version')
...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div class="flex items-center gap-4">
            <a href="{{ route('routes.show', $route->id) }}" class="w-10 h-10 rounded-full flex items-center justify-center text-gray-500 bg-gray-100 shadow-[4px_4px_8px_#d1d5db,-4px_-4px_8px_#ffffff] active:shadow-[inset_2px_2px_4px_#d1d5db,inset_-2px_-2px_4px_#ffffff] transition-all hover:text-blue-600">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
            </a>
            <p class="text-gray-500 text-lg font-medium">Create New Version: {{ $route->route_code }}</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8" x-data="routeManager()">
        ...
        <div class="lg:col-span-1">
            <x-card>
                <form id="routeForm" action="{{ route('routes.version.store', $route->id) }}" method="POST" class="space-y-6">
                    @csrf
                    <input type="hidden" name="waypoints" x-model="waypointsJson">

                    <!-- Read-only master route data -->
                    <div class="p-5 bg-gray-100/50 rounded-2xl border border-gray-200/60 shadow-[inset_2px_2px_4px_rgba(0,0,0,0.02)] space-y-4 mb-6">
                        <div class="flex justify-between items-center pb-3 border-b border-gray-200/70">
                            <span class="text-xs font-bold text-gray-500 uppercase tracking-widest">Route Type</span>
                            <span class="text-sm text-blue-600 font-black uppercase tracking-wider bg-blue-100/50 px-3 py-1 rounded-lg" x-text="routeType"></span>
                        </div>
                        <div>
                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Origin</p>
                            <p class="text-sm font-bold text-gray-800 leading-snug">{{ $route->origin_name }}</p>
                        </div>
                        <div>
                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Destination</p>
                            <p class="text-sm font-bold text-gray-800 leading-snug">{{ $route->destination_name }}</p>
                        </div>
                    </div>
    ...
    @php
        ...
        $initialWaypoints = $latestVersion ? json_encode($latestVersion->waypoints) : '[]';
    @endphp
    <script>
        function routeManager() {
            return {
                ...
                removeWaypoint(index) {
                    if (index === 0) {
                        Toastify({text: "Titik Origin tidak boleh dihapus.", duration: 3000, style:{background:"#fee2e2", color:"#991b1b"}}).showToast();
                        return;
                    }
                    if (index === this.waypoints.length - 1 && this.waypoints.length > 2) {
                        Toastify({text: "Titik Destination tidak boleh dihapus.", duration: 3000, style:{background:"#fee2e2", color:"#991b1b"}}).showToast();
                        return;
                    }
                    if (index === this.waypoints.length - 1 && this.waypoints.length === 2) {
                         Toastify({text: "Minimal harus ada Origin dan Destination.", duration: 3000, style:{background:"#fee2e2", color:"#991b1b"}}).showToast();
                         return;
                    }
