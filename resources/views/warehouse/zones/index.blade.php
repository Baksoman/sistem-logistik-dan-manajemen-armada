...
@section('content')
    ...
    <div x-data="{ 
            ...
            editData: { id: '', warehouse_id: '', name: '', description: '' }
         }" 
        ...
        <x-card class="mb-8">
            <h3 class="text-xl font-bold text-gray-800 mb-6">Zone List</h3>
                ...
                <table class="w-full text-left border-collapse min-w-max whitespace-nowrap">
                    <thead>
                        <tr class="border-b border-gray-300 text-gray-500 text-sm tracking-widest uppercase">
                            ...
                            <th class="py-4 px-4 font-bold">Zone Name</th>
                    ...
                    <tbody class="text-gray-700 font-medium">
                        @forelse($zones as $zone)
                            <tr class="border-b border-gray-200/50 hover:bg-gray-200/30 transition">
                                <td class="py-4 px-4">{{ $zone->warehouse->name ?? '-' }}</td>
                                <td class="py-4 px-4 font-bold text-gray-800">{{ $zone->name }}</td>
                                <td class="py-4 px-4 text-sm text-gray-500">{{ $zone->description ?? '-' }}</td>
                                <td class="py-4 px-4">
                                    <div class="flex items-center justify-center gap-3">
                                        <button type="button" @click="$dispatch('open-edit', { id: '{{ $zone->id }}', warehouse_id: '{{ $zone->warehouse_id }}', name: '{{ $zone->name }}', description: '{{ $zone->description }}' })" class="w-10 h-10 rounded-full flex items-center justify-center text-blue-500 bg-gray-100 shadow-[4px_4px_8px_#d1d5db,-4px_-4px_8px_#ffffff] hover:shadow-[inset_2px_2px_4px_#d1d5db,inset_-2px_-2px_4px_#ffffff] transition-all">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                        </button>
                                        <form id="delete-form-{{ $zone->id }}" action="{{ route('warehouse.zones.destroy', $zone->id) }}" method="POST" class="inline">
                                            @csrf @method('DELETE')
                                            <button type="button" onclick="confirmDelete('delete-form-{{ $zone->id }}')" class="w-10 h-10 rounded-full flex items-center justify-center text-red-500 bg-gray-100 shadow-[4px_4px_8px_#d1d5db,-4px_-4px_8px_#ffffff] hover:shadow-[inset_2px_2px_4px_#d1d5db,inset_-2px_-2px_4px_#ffffff] transition-all">
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="py-8 text-center text-gray-400">No zones found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <!-- Create Slide-Over -->
        <x-slide-over title="Add New Zone">
            <form action="{{ route('warehouse.zones.store') }}" method="POST" class="space-y-6">
                @csrf
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Warehouse</label>
                    <select name="warehouse_id" required class="w-full bg-gray-100 rounded-2xl px-5 py-4 font-medium text-gray-600 shadow-[inset_4px_4px_8px_#d1d5db,inset_-4px_-4px_8px_#ffffff] border-none focus:ring-0 focus:outline-none appearance-none select2-enable">
                ...
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Zone Name</label>
                    <x-input type="text" name="name" required placeholder="e.g. Zone A" />
                </div>
                <div>
                    ...
                    <div class="pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-10">
                        <div class="pointer-events-auto w-screen max-w-md transform transition-transform duration-500 ease-in-out bg-gray-100 shadow-2xl flex flex-col"
                            ...
                            <div class="flex items-center justify-between px-6 py-6 border-b border-gray-300">
                                <h2 class="text-xl font-bold text-gray-800 tracking-wide" id="slide-over-title">Edit Zone</h2>
                            ...
                            <div class="relative flex-1 px-6 py-6 overflow-y-auto">
                                <form :action="`{{ route('warehouse.zones.index') }}/${editData.id}`" method="POST" class="space-y-6">
                                    @csrf @method('PUT')
                                    <div>
                                        <label class="block text-sm font-bold text-gray-700 mb-2">Warehouse</label>
