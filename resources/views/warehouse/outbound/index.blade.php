...
@section('content')
    ...
    <div x-data="Object.assign({}, dataTable({
            endpoint: '/api/search/inventory-movements?force_type=outbound',
            ...
            startScanner() {
                ...
                this.$nextTick(() => {
                    const scanner = new Html5Qrcode('outbound-barcode-reader');
                    window.__outboundScanner = scanner;
                    scanner.start(
            ...
            stopScanner() {
                if (window.__outboundScanner) {
                    window.__outboundScanner.stop().then(() => { this.scannerActive = false; }).catch(() => { this.scannerActive = false; });
                } else {
                    this.scannerActive = false;
                }
            },
            handleScannedCode(code) {
                ...
                if (found) {
                    ...
                    Toastify({ text: 'Item found: ' + found.sku + ' - ' + found.name + ' (Stock: ' + found.qty + ')', duration: 3000, gravity: 'top', position: 'right', style: { background: '#10b981', borderRadius: '12px', fontWeight: 'bold' } }).showToast();
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-8">
            ...
            <div class="flex flex-col lg:flex-row w-full lg:w-auto gap-3">
                <a href="{{ route('warehouse.outbound.export.excel') }}" class="flex items-center justify-center gap-2 px-5 py-3 rounded-2xl font-bold text-emerald-700 bg-gray-100 shadow-[4px_4px_8px_#d1d5db,-4px_-4px_8px_#ffffff] active:shadow-[inset_2px_2px_4px_#d1d5db] transition-all hover:bg-emerald-50">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                    Excel
                </a>
                <a href="{{ route('warehouse.outbound.export.pdf') }}" class="flex items-center justify-center gap-2 px-5 py-3 rounded-2xl font-bold text-red-600 bg-gray-100 shadow-[4px_4px_8px_#d1d5db,-4px_-4px_8px_#ffffff] active:shadow-[inset_2px_2px_4px_#d1d5db] transition-all hover:bg-red-50">
        ...
        <x-filter-modal title="Filter Outbound Records">
            <div>
                ...
                <tbody class="text-gray-700 font-medium">
                    <template x-for="movement in data" :key="movement.id">
                        <tr class="border-b border-gray-200/50 hover:bg-gray-200/30 transition">
                            ...
                            <td class="py-4 px-4">
                                <span class="px-3 py-1 text-xs font-bold rounded-full shadow-[inset_2px_2px_4px_#d1d5db,inset_-2px_-2px_4px_#ffffff] text-red-700 bg-red-50" x-text="`-${new Intl.NumberFormat().format(movement.quantity)}`">
                                </span>
                            </td>
        ...
        <template x-if="scannerActive">
            <div class="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm" @click.self="stopScanner()">
                <div class="bg-gray-100 rounded-3xl shadow-2xl p-6 w-full max-w-md mx-4">
                    ...
                    <div id="outbound-barcode-reader" class="rounded-2xl overflow-hidden shadow-[inset_4px_4px_8px_#d1d5db,inset_-4px_-4px_8px_#ffffff]"></div>
