...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans text-gray-900 antialiased bg-gray-100 min-h-screen flex flex-col pt-28 selection:bg-gray-300 selection:text-gray-900">
        ...
        <div x-data="{ scrolled: false, mobileMenuOpen: false }" @scroll.window="scrolled = (window.pageYOffset > 40)"
            ...
            <nav class="bg-gray-100 flex justify-between items-center transition-all duration-500 ease-in-out relative"
                ...
                <!-- Logo -->
                <div class="flex items-center gap-3 transition-transform duration-500" :class="scrolled ? 'scale-90' : 'scale-100'">
                    <div class="w-10 h-10 rounded-full shadow-[inset_3px_3px_6px_#d1d5db,inset_-3px_-3px_6px_#ffffff] flex items-center justify-center font-bold text-xl text-gray-800">
                        L
                    </div>
                ...
                <div class="hidden md:flex items-center gap-4 transition-all duration-500">
                    <a href="{{ url('/') }}" class="text-base font-semibold text-gray-600 hover:text-gray-900 transition-all duration-300 px-4 py-2 rounded-xl hover:shadow-[inset_3px_3px_6px_#d1d5db,inset_-3px_-3px_6px_#ffffff]">Home</a>
                    <a href="#" class="text-base font-semibold text-gray-600 hover:text-gray-900 transition-all duration-300 px-4 py-2 rounded-xl hover:shadow-[inset_3px_3px_6px_#d1d5db,inset_-3px_-3px_6px_#ffffff]">About</a>
                    
                    @if (Route::has('login'))
                        @auth
                            <a href="{{ url('/dashboard') }}" class="text-base font-semibold text-gray-700 hover:text-gray-900 transition-all duration-300 px-5 py-2 rounded-xl shadow-[4px_4px_8px_#d1d5db,-4px_-4px_8px_#ffffff] hover:shadow-[6px_6px_12px_#c2c6cc,-6px_-6px_12px_#ffffff] hover:-translate-y-0.5 active:shadow-[inset_3px_3px_6px_#d1d5db,inset_-3px_-3px_6px_#ffffff] active:translate-y-0 ml-2">Dashboard</a>
                        @else
                            <a href="{{ route('login') }}" class="text-base font-semibold text-gray-700 hover:text-gray-900 transition-all duration-300 px-6 py-2 rounded-xl shadow-[4px_4px_8px_#d1d5db,-4px_-4px_8px_#ffffff] hover:shadow-[6px_6px_12px_#c2c6cc,-6px_-6px_12px_#ffffff] hover:-translate-y-0.5 active:shadow-[inset_3px_3px_6px_#d1d5db,inset_-3px_-3px_6px_#ffffff] active:translate-y-0 ml-2">Log in</a>
                        @endauth
                    @endif
                </div>
                ...
                <div class="md:hidden flex items-center">
                    <button @click="mobileMenuOpen = !mobileMenuOpen" 
                        ...
                        <svg x-show="mobileMenuOpen" class="w-5 h-5" x-cloak fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                    </button>
                </div>
                ...
                <div x-show="mobileMenuOpen" 
                    ...
                    <a href="{{ url('/') }}" class="text-center font-bold text-gray-700 hover:text-gray-900 px-4 py-3 rounded-2xl shadow-[inset_4px_4px_8px_#d1d5db,inset_-4px_-4px_8px_#ffffff] bg-gray-100">Home</a>
                    <a href="#" class="text-center font-bold text-gray-600 hover:text-gray-900 px-4 py-3 rounded-2xl shadow-[4px_4px_8px_#d1d5db,-4px_-4px_8px_#ffffff] hover:shadow-[inset_4px_4px_8px_#d1d5db,inset_-4px_-4px_8px_#ffffff] transition-all bg-gray-100">About</a>
                    
                    <div class="h-px w-full bg-gray-300 shadow-[0_1px_0_#ffffff] my-1"></div>

                    @if (Route::has('login'))
                        @auth
                            <a href="{{ url('/dashboard') }}" class="text-center font-extrabold text-gray-800 px-4 py-3 rounded-2xl shadow-[6px_6px_12px_#d1d5db,-6px_-6px_12px_#ffffff] active:shadow-[inset_4px_4px_8px_#d1d5db,inset_-4px_-4px_8px_#ffffff] transition-all bg-gray-100">Dashboard</a>
                        @else
                            <a href="{{ route('login') }}" class="text-center font-extrabold text-gray-800 px-4 py-3 rounded-2xl shadow-[6px_6px_12px_#d1d5db,-6px_-6px_12px_#ffffff] active:shadow-[inset_4px_4px_8px_#d1d5db,inset_-4px_-4px_8px_#ffffff] transition-all bg-gray-100">Log in</a>
                        @endauth
                    @endif
                </div>
            </nav>
