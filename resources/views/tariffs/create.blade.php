...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <h1 class="text-2xl font-black text-gray-800 tracking-wider">Create New Tariff</h1>
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('tariffs.store') }}" method="POST" class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        @csrf
        ...
        <x-card class="space-y-6">
            ...
            <div class="flex items-center gap-3 pt-4 border-t border-gray-100">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="w-5 h-5 rounded text-blue-600 focus:ring-blue-500 border-gray-300 shadow-sm">
        ...
        <x-card class="space-y-6">
            ...
            <div>
                ...
                <input type="number" step="0.01" name="fixed_price" value="{{ old('fixed_price', 0) }}" required class="w-full bg-gray-100 rounded-xl px-4 py-3 font-mono font-bold text-gray-700 border-none focus:ring-2 focus:ring-blue-500 shadow-[inset_2px_2px_4px_#d1d5db,inset_-2px_-2px_4px_#ffffff]">
            ...
            <div>
                ...
                <input type="number" step="0.01" name="price_per_km" value="{{ old('price_per_km', 0) }}" required class="w-full bg-gray-100 rounded-xl px-4 py-3 font-mono font-bold text-gray-700 border-none focus:ring-2 focus:ring-blue-500 shadow-[inset_2px_2px_4px_#d1d5db,inset_-2px_-2px_4px_#ffffff]">
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Price per KG (Rp / KG)</label>
                    <input type="number" step="0.01" name="price_per_kg" value="{{ old('price_per_kg', 0) }}" required class="w-full bg-gray-100 rounded-xl px-4 py-3 font-mono font-bold text-gray-700 border-none focus:ring-2 focus:ring-blue-500 shadow-[inset_2px_2px_4px_#d1d5db,inset_-2px_-2px_4px_#ffffff]">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Price per CBM (Rp)</label>
                    <input type="number" step="0.01" name="price_per_cbm" value="{{ old('price_per_cbm', 0) }}" required class="w-full bg-gray-100 rounded-xl px-4 py-3 font-mono font-bold text-gray-700 border-none focus:ring-2 focus:ring-blue-500 shadow-[inset_2px_2px_4px_#d1d5db,inset_-2px_-2px_4px_#ffffff]">
                </div>
            </div>
